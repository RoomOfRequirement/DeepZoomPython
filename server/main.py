...
class _SlideCache:
    ...
    def get(self, path: Path) -> AnnotatedDeepZoomGenerator:
        ...
        if (slide._metadata.get("mm_x") is not None
            and slide._metadata.get("mm_y") is not None):
            slide.mpp = (slide._metadata["mm_x"] + slide._metadata["mm_y"]) * (10**3) / 2
    ...
    def _get_transform(self, color_profile: ImageCms.ImageCmsProfile) -> Transform:
...
class _Directory:
    ...
    def __init__(self, slidedir: Path, relpath: PurePath = _DEFAULT_RELPATH):
        ...
        for cur_path in sorted((slidedir / relpath).iterdir()):
            ...
            elif DeepZoomGenerator.canRead(cur_path):
...
def create_app(
    ...
    def index(request: Request):
        return templates.TemplateResponse('files.html', {'request': request, 'root_dir': _Directory(app.slidedir)})
...
if __name__ == '__main__':
    ...
    if config.get('log_config_file_path'):
        with open(config['log_config_file_path'], 'r') as f:
